...
const bootSequence = [
  ...
  { text: 'Loading OS v3.14 ...', delay: 50 },
  { text: 'Mounting /dev/stanley ...\u000A', delay: 750 },
  { text: '\u000APCI Device Listing ...', delay: 750 },
  {
    text: 'Bus\u00A0\u00A0Device\u00A0\u00A0ID\u00A0\u00A0\u00A0\u00A0Device Class',
    delay: 300,
  },
  {
    text: '───\u00A0\u00A0──────\u00A0\u00A0────\u00A0\u00A0───────────────────────',
  ...
  {
    text: `0${'\u00A0'.repeat(4)}37${'\u00A0'.repeat(6)}24C2${'\u00A0'.repeat(
      2,
    )}体制  Neural Network  界面`,
    delay: 50,
  },
  {
    text: `0${'\u00A0'.repeat(4)}23${'\u00A0'.repeat(6)}24C4${'\u00A0'.repeat(
      2,
    )}ネットワーク  Controller`,
    delay: 50,
  },
  {
    text: `1${'\u00A0'.repeat(4)}08${'\u00A0'.repeat(6)}5F33${'\u00A0'.repeat(
      2,
    )}System Peripherals`,
    delay: 50,
  },
  {
    text: `1${'\u00A0'.repeat(4)}04${'\u00A0'.repeat(6)}4F34${'\u00A0'.repeat(
      2,
    )}特異点`,
    delay: 50,
  },
...
const BootSequence = ({ onComplete }: { onComplete: () => void }) => {
  const [charStates, setCharStates] = useState<
    { char: string | null; isGlitching: boolean }[]
  >(() => Array(fullText.length).fill({ char: null, isGlitching: false }));
  const [showCursor, setShowCursor] = useState(true);
  const [frame, setFrame] = useState(0);

  // Sequence state management
  const [phase, setPhase] = useState<'bootCode' | 'startup' | 'boot'>(
    'bootCode',
  );
    ...
    if (phase === 'bootCode' && currentLineIndex < bootCodeSnippets.length) {
      const timer = setTimeout(() => {
        setDisplayLines((prev) => [
          ...prev,
          bootCodeSnippets[currentLineIndex],
        ]);
      ...
      phase === 'bootCode' &&
      currentLineIndex >= bootCodeSnippets.length
    ) {
  ...
  useEffect(() => {
    if (phase === 'startup' && currentLineIndex < startupCodeSequence.length) {
      const timer = setTimeout(() => {
        setDisplayLines((prev) => [
          ...prev,
          startupCodeSequence[currentLineIndex],
        ]);
        setCurrentLineIndex((prev) => prev + 1);
      }, 50); // time each line
      ...
      phase === 'startup' &&
      currentLineIndex >= startupCodeSequence.length
    ) {
      const transitionTimer = setTimeout(() => {
        setDisplayLines([]);
        setCurrentLineIndex(0);
  ...
  useEffect(() => {
    if (phase === 'boot' && currentLineIndex < bootSequence.length) {
      const timer = setTimeout(() => {
        setDisplayLines((prev) => [
          ...prev,
          bootSequence[currentLineIndex].text,
        ]);
        setCurrentLineIndex((prev) => prev + 1);
